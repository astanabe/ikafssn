...
- Template dispatch on `KmerInt` type; CLI-side dispatch uses `kmer_type_for(k, t)` (bit-width-based: `2*k` for contiguous, `2*k+1` for spaced seeds; >16 bits → `uint32_t`); reader-side dispatch reads `kmer_type` from index header. No virtual calls in hot loops
