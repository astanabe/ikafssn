...
- **`util/`** — CLI parser, size string parser ("8G"), socket utilities, progress display, logger, common CLI init helpers (`check_version()`, `make_logger()`, `resolve_threads()` in `common_init.hpp`), context parameter parser (`context_parser.hpp`)
