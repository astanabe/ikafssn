...
|---|---|---|
| `ikafssnindex` | Build k-mer inverted index from BLAST DB (`-mode 1` skips `.kpx`; `-template_type both` builds coding and optimal indexes sequentially; `-freq_threshold_part` controls .kpx v7 per-(kmer, seq_id) partitioning, default 8, max 255) | NCBI C++ Toolkit, TBB |
...
- **`index/`** — Reader/writer for four index file formats (`.kix` main index, `.kpx` position data, `.ksx` sequence metadata, `.khx` build-time exclusion bitset).  All four files share format version 7 (Phase 5i — single major version policy, all index files bump together; users only have to track one number).  `.kix` v7 (magic `"KIX7"`, 96 B header) stores the **distinct seq_id** delta stream — intra-sequence k-mer duplicates are removed at build time by a SIMD dedup kernel (`src/index/seq_id_dedup.{hpp,cpp}` runtime dispatcher + `seq_id_dedup_tier.cpp` per-tier implementation, mirroring the FastPFor tier dispatch).  This realigns `-max_freq_build` and `-stage1_max_freq` with the original "matched-entry-count" semantic: thresholds now select on **the number of containing sequences**, not total occurrences.  `.kpx` v7 (magic `"KPX7"`, 64 B header) keeps the per-(kmer, seq_id) partition + short bucket structure but the short bucket is now **self-describing** (carries its own delta-encoded seq_id list and per-seq_id u8 occurrence counts), so decoding is candidate-set-driven (`open_stream_kpx_for_candidates(...)` takes a sorted candidate seq_id array and returns per-candidate position vectors) instead of lock-step against the .kix stream.  Both `.kix` and `.kpx` payloads still go through `src/index/pfd_codec.{hpp,cpp}` (runtime dispatcher) + `pfd_codec_tier.cpp` (per-tier implementation): `.kix` payload is FastPFor's `CompositeCodec<SIMDFastPFor<4>, VariableByte>` (PForDelta with VByte exception stream); each partition group and the short bucket FOR-within-block stream (per-128-element block stores its min and the bitpacked spread `(value - min)`) drives the position layout.  Bitpacking primitives come from FastPFor v0.4.0 (`simdpackwithoutmask` / `simdunpack`), recompiled into per-ISA OBJECT libraries under different `FastPForLib_<tier>` namespace remappings.  On x86_64 the four tiers `ikafssn_pfd_{sse42,avx2,avx512bw,avx512vbmi2}` are built (Phase 5f 4-tier ladder: SSE4.2 is the floor, Scalar is rejected at startup with `exit(2)`).  On aarch64 a single `ikafssn_pfd_neon` tier is built that compiles the same `pfd_codec_tier.cpp` source via SIMDe (https://github.com/simd-everywhere/simde), translating its SSE intrinsics to NEON; SVE / SVE2 capable CPUs are routed to the same NEON tier object (Phase 5h, single-tier design — a SVE-native bitpacker would be orthogonal hand-coded work and is not implemented).  The runtime dispatcher in `pfd_codec.cpp` selects the per-arch tier at first use based on `current_simd_cap()`.  Codec selection follows `format_version`; v6-or-older indexes are rejected at open with a clear "rebuild your index" message.  `.kix` keeps sentinel-based offsets (`offsets[table_size+1]`, posting byte length = `offsets[kmer+1]-offsets[kmer]`); both `.kix` and `.kpx` support conditional uint32_t offsets when posting data < 4 GiB (`KIX_FLAG_OFFSET32` flag / `KpxHeader.offset_type` field), halving dictionary size.  `KixReader`/`KpxReader` use dual pointers (`offsets32_`/`offsets64_`) with inline branch dispatch.  `KixReader::count_postings(kmer)` is an O(1) 4-byte read of the leading `distinct_count` field; `bulk_count_postings()` runs in O(table_size).  Table index types use `uint32_t` (max table size = 4^12 = 16M).  Index builder uses a partition+buffer strategy (`skip_kpx` config flag omits `.kpx` for mode 1 indexes); builder rewrites files at finalize to select optimal offset width.  For spaced seeds (`-t > 0`), builder uses `scan_spaced()`; `-template_type both` builds coding and optimal indexes sequentially (one after the other).  All readers expose `willneed_size()` / `apply_madvise(bool)` for budget-based madvise allocation (used by `-memory_limit` in search/server).  v3 readers/writers are fully removed; users upgrading from v3 must reindex
...
- **`.kvx`** — Text manifest file for volume discovery (naming: `<db_base>.<kk>mer.kvx`). Header lines include `FORMAT_VERSION 7` (informational; the parser ignores unknown lines for forward compatibility) followed by `TITLE` and `DBLIST "vol0" "vol1" ...`.  Always generated by `ikafssnindex`.  Readers use `.kvx` to discover volumes
- **`.kix`** — Magic `"KIX7"`, 96 B header, direct-address offset table (offsets[4^k + 1] with sentinel, uint32 or uint64 depending on posting data size).  Per-k-mer payload format: `[u32 distinct_count][u32 payload_words][u32 payload[N]]` where the payload is FastPFor's `CompositeCodec<SIMDFastPFor<4>, VariableByte>` output over the **distinct seq_id** delta stream `[abs_first, d1, d2, ...]` with `d_i >= 1` (cumsum at decode reconstructs absolute distinct seq_ids).  Intra-sequence k-mer duplicates are removed at build time by the SIMD dedup kernel (`seq_id_dedup_tier.cpp`).  PForDelta's exception stream handles the wide-value `abs_first` while the per-block bit-width tracks the typically-small consecutive deltas.  `distinct_count` available in O(1) from the leading u32 of each posting.  Header carries `total_distinct_postings` (sum across k-mers).  `-max_freq_build` and `-stage1_max_freq` threshold by `distinct_count` (i.e. the number of sequences containing the k-mer)
- **`.kpx`** — Magic `"KPX7"`, 64 B header (Phase 5i layout: per-(kmer, seq_id) partition groups + self-describing short bucket), pos_offsets[4^k] (uint32 or uint64 depending on posting data size).  Per-k-mer payload format: `[u32 distinct_count][u32 position_count][u32 partition_count]` then for each partition group (sorted by seq_id) `[u32 seq_id][u32 occurrence_count][FOR-block stream]`, then `[u32 short_seq_count][u32 short_position_count]` and (when short_seq_count > 0) `[u32 abs_first_seq_id][varint delta_seq_id × (short_seq_count-1)][u8 occ_count × short_seq_count][FOR-block stream over short_position_count positions]`.  Each FOR-block stream is `N × {[u8 b][u32 min][128*b/8 bytes bitpacked (value - min)]}` followed by `[u8 tail_count]` plus an optional `[u32 tail_min][varint tail-shifted]` when tail_count > 0.  A (k-mer, seq_id) cluster whose occurrence count exceeds the build-time `-freq_threshold_part` (default 8, `>` comparison; max 255 since occ_count is u8) gets its own partition group; the rest merge into the short bucket.  Decoding is candidate-set-driven (`pfd::open_stream_kpx_for_candidates(posting, bytes, candidates, n_candidates, out)` returns per-candidate position vectors) — no lock-step dependency on the .kix stream.  Header carries `total_position_count` (sum across k-mers).  Not generated when `ikafssnindex -mode 1` is used; `discover_volumes()` sets `has_kpx=false` for such indexes, and search/server/info commands handle the absence gracefully
- **`.ksx`** — Magic `"KMSX"`, format_version 7 (data layout unchanged from v3).  Header + seq_lengths[] + accession string table (enables standalone result display without BLAST DB)
- **`.khx`** — Magic `"KMHX"`, format_version 7 (data layout unchanged from v3).  32 B header + bitset (ceil(4^k / 8) bytes).  Shared across all volumes (one per k value, naming: `<db_base>.<kk>mer.khx`).  Generated only when `-max_freq_build` is set to a value other than 1 (disabled).  K-mer **distinct seq_id counts** are aggregated across all volumes before applying the threshold (Phase 5i: aligned with .kix's distinct-count semantic).  Records which k-mers were excluded during index build (bit i = 1 means k-mer i was excluded).  Used at search time for fractional `-stage1_min_score` threshold adjustment
