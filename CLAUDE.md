...
- **`index/`** — Reader/writer for four index file formats (`.kix` main index, `.kpx` position data, `.ksx` sequence metadata, `.khx` build-time exclusion bitset).  All four files share format version 4. `.kix` v4 (magic `"KIX4"`, 96 B header) and `.kpx` v4 (magic `"KPX4"`, 64 B header) compress per-k-mer postings via SIMD-FastPFOR* + Simple-8b/VByte-tail (FastPFor v0.4.0 `simdfastpfor128`, block size 128) wrapped by `pfd_codec` (`src/index/pfd_codec.{hpp,cpp}`).  On-disk per-posting layout: `[u32 count][u32 payload_words][uint32 payload[payload_words]]`.  For `.kix` the payload encodes the seq_id **delta** stream (first abs, then differences); for `.kpx` it encodes **absolute** positions (FastPFor's per-block bit-width adapts to the position range, replacing v3's within-seq delta-reset bookkeeping).  FastPFor codec instances are `thread_local` because the encode/decode paths hold mutable scratch state.  `.kix` keeps sentinel-based offsets (`offsets[table_size+1]`, posting byte length = `offsets[kmer+1]-offsets[kmer]`); both `.kix` and `.kpx` support conditional uint32_t offsets when posting data < 4 GiB (`KIX_FLAG_OFFSET32` flag / `KpxHeader.offset_type` field), halving dictionary size.  `KixReader`/`KpxReader` use dual pointers (`offsets32_`/`offsets64_`) with inline branch dispatch.  `KixReader::count_postings(kmer)` is now an O(1) 4-byte read of the leading `count` field (vs O(N) varint walk in v3); `bulk_count_postings()` runs in O(table_size).  Table index types use `uint32_t` (max table size = 4^12 = 16M).  Index builder uses a partition+buffer strategy (`skip_kpx` config flag omits `.kpx` for mode 1 indexes); builder rewrites files at finalize to select optimal offset width.  For spaced seeds (`-t > 0`), builder uses `scan_spaced()`; `-template_type both` builds coding and optimal indexes sequentially (one after the other).  All readers expose `willneed_size()` / `apply_madvise(bool)` for budget-based madvise allocation (used by `-memory_limit` in search/server).  v3 readers/writers are fully removed; users upgrading from v3 must reindex
...
- **`.kix`** — Magic `"KIX4"`, 96 B header, direct-address offset table (offsets[4^k + 1] with sentinel, uint32 or uint64 depending on posting data size), then per-k-mer SIMD-FastPFOR* + Simple-8b/VByte payload (`[u32 count][u32 payload_words][uint32 payload[N]]`).  `count` derived in O(1) from the leading u32 of each posting, so no counts table is required.  Encodes delta-compressed seq_id stream
- **`.kpx`** — Magic `"KPX4"`, 64 B header, pos_offsets[4^k] (uint32 or uint64 depending on posting data size), then per-k-mer SIMD-FastPFOR* + Simple-8b/VByte payload (same `[count][payload_words][payload]` framing as `.kix`).  Encodes **absolute** positions; sequence-boundary delta reset is no longer required.  Not generated when `ikafssnindex -mode 1` is used; `discover_volumes()` sets `has_kpx=false` for such indexes, and search/server/info commands handle the absence gracefully
