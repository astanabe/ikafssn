...
// Sort `buf` by (kmer_value, seq_id, pos). On x86_64 with AVX2+ uses Intel
// x86-simd-sort's `keyvalue_qsort<uint64_t, uint32_t>` to sort the
// (kmer_value << 32 | seq_id) keys against `pos` values, followed by a local
// pos-only sort within each (kmer, seq_id) block; otherwise a
// `tbb::parallel_sort` with a 3-key comparator. Both paths give bit-exact
// output because TempEntry tuples are unique per index build.
