...
- If `-db` is specified, the count must match the number of `-ix` flags (paired in order). Databases without a `-db` override default to the `-ix` prefix as the BLAST DB path. Mode 3 (max_mode=3) needs a BLAST DB that actually exists at that path; when none is found the server logs that mode 3 is unavailable for that database and serves it at max_mode=2, so an index-only prefix starts normally.
