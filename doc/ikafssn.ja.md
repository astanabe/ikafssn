...
オプション:
  ...
  -stage1_topn <int>      Stage 1 候補数上限 (同点含む)、0=無制限 (デフォルト: 0)
...
オプション:
  ...
  -stage1_topn <int>      デフォルト Stage 1 候補数上限 (同点含む) (デフォルト: 0)
...
オプション:
  ...
  -stage1_topn <int>       Stage 1 候補数上限 (同点含む) (デフォルト: サーバ側デフォルト)
...
1. **Stage 1 (候補選択):** クエリの各 k-mer に対して ID ポスティングをスキャンし、配列ごとにスコアを集計します。スコア種別は 2 種類あります: **coverscore** (配列にマッチしたクエリ k-mer の種類数) と **matchscore** (クエリ k-mer と参照配列位置の総マッチ数)。`stage1_min_score` 以上のスコアを持つ配列を候補として選出します。`stage1_topn > 0` の場合はスコア上位 N 件を返しますが、同点を含みます: N 番目のスコアと同点の候補はすべて保持されるため、返却件数が N を超えることがあります。`stage1_topn = 0` (デフォルト) の場合は全候補を返します。
