...
- BLAST DB が `makeblastdb -parse_seqids` 付きで構築されている必要があります。アクセッションの逆引き索引を作るのがこのオプションです (v5 では LMDB と並ぶ `.nos`、v4 では `.nsi` / `.nsd` の文字列 ISAM)。NCBI が配布するデータベースはこの条件を満たします。このオプションなしで構築したデータベースはアクセッションの代わりに `BL_ORD_ID:<n>` というプレースホルダを持つため、そもそも検索出力に解決すべきアクセッションがありません。またインデックス構築に使ったものと同じデータベースである必要があり、別のデータベースではアクセッションを解決できません。
...
- 同時に使うファイルディスクリプタ数は、データベースのボリューム数によらず 10 本未満の定数です。
...
- `-db` を指定する場合、その数は `-ix` の数と一致する必要があります (順番に対応)。`-db` を省略した DB は `-ix` プレフィックスを BLAST DB パスとして使用します。モード 3 (max_mode=3) にはそのパスに BLAST DB が実在することが必要で、見つからない場合はその DB のモード 3 が利用不可である旨をログに出して max_mode=2 で提供します。したがって索引のみのプレフィックスでも通常どおり起動します。
...
**マルチアクセッション defline:** 元の BLAST DB が `makeblastdb -parse_seqids` で構築され、`\x01` (`^A`) 区切りの multi-defline レコード（同一塩基配列を複数アクセッションで登録する NCBI 慣習）を含んでいる場合、`ikafssnindex` は OID ごとに**全てのアクセッション**を保持します。`.ksx` のアクセッション文字列は OID ごとに全アクセッションを `\x01` で連結した形で格納され、検索出力 (`sseqid` カラム / SAM RNAME / プロトコルの `sseqid` フィールド) も同じ `\x01` 連結形のまま emit されます。`ikafssnretrieve` は FASTA defline の `sseqid=` フィールドで連結形を保持し、レコード ID は 1 個のトークンに保ちます。受け取り側で `\x01` を区切りとして分割してください。`-seqidlist` フィルタと `ikafssnretrieve` は `\x01` 連結形・個別アクセッションのいずれを指定しても解決できます。
