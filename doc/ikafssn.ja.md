...
オプション:
  ...
  -overlap_length <int>   隣接フラグメントの末尾オーバーラップ塩基数
                          ...
                          ことを保証します。
  -memory_limit <size>    構築時メモリ予算 (デフォルト: 物理メモリの半分)
                          接尾辞 K, M, G を認識。パーティション数の自動決定と、
                          メタデータパス・ポスティングパスで同時処理する
                          ボリューム数の決定 (各バッチの合計コストがこの予算
                          に収まるよう自動分割) の両方に使用される。
