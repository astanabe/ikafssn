...
**インデックスフォーマットバージョン:** スペースドシードインデックスはフォーマットバージョン 2 を使用します (`.kix`/`.kpx`/`.ksx`/`.khx` ヘッダにテンプレート長 `t` とテンプレート種別フィールドが含まれます)。これらの v2 インデックスファイルは、フォーマットバージョン 1 のみをサポートする旧バージョンの ikafssn とは互換性がありません。連続インデックス (t=0) もフォーマットバージョン 2 を使用しますが、構造的には後方互換性があります。
